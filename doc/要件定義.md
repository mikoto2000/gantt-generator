...
| status | string | | `cancelled` / `中止` で中止扱い |
| progress | 0-100(%) | | 進捗率（0-100、末尾に `%` も可） |
...
| 日本語ヘッダ（例）タスク名/開始/終了/期間/依存/実績開始/実績終了/実績期間/状態/進捗/備考 | | | 英語ヘッダと同義で使用可能 |
...
- 循環依存 → エラー
- 全フィールド空の行は無視（エラーにしない）
- 出力上のタスク順は CSV の記述順を維持する
...
ganttgen <input.csv>
# 監視付き
ganttgen --watch <input.csv>
# 監視 + LiveReload
ganttgen --livereload <input.csv>
```
...
--all-workdays          週末や祝日を稼働日として扱う（--holidays 指定時も含む）
--gen-template       sample/sample.csv と同じヘッダの空 CSV を出力して終了
--watch              CSV を1秒間隔で監視し、変更時に再生成
--livereload         LiveReload サーバを起動し、生成 HTML にクライアントを埋め込む（監視も有効化）
--livereload-port    LiveReload 用ポート（デフォルト: 35729）
```

### デフォルト出力
入力 CSV と同じディレクトリに `gantt.html` を出力する。
